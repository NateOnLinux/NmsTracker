...
public readonly record struct PlayerCoordinates
{
    ...
    {
        if (x is > 2047 or < -2048) throw new ArgumentOutOfRangeException(nameof(x), "X coordinate must be in range -2048 to 2047");
        if (z is > 2047 or < -2048) throw new ArgumentOutOfRangeException(nameof(z), "Z coordinate must be in range -2048 to 2047");
        if (ss > 4095) throw new ArgumentOutOfRangeException(nameof(ss), "Solar System Index must be in range 0 to 4095");
        if (p > 15) throw new ArgumentOutOfRangeException(nameof(p), "Planet Index must be in range 0 to 15");
        X = x;
        Z = z;
        Y = y;
        G = g;
