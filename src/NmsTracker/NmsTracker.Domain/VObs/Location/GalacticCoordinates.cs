...
public readonly record struct GalacticCoordinates
{
    ...
    /// <summary>
    /// Regional Z Coordinate in range 0 &#8804; value &#8804; 4095
    /// </summary>
    public ushort Z { get; }
    ...
    /// <summary>
    /// 4 Bit Planet Index and 12 Bit Solar System Index<br/>
    /// See the No Man's Sky Wiki for more information about
    ...
    {
        if (x > 4095) throw new ArgumentOutOfRangeException(nameof(x), x, "X coordinate must be in range 0 to 4095");
        if (z > 4095) throw new ArgumentOutOfRangeException(nameof(z), z, "Z coordinate must be in range 0 to 4095");
        X = x;
        Z = z;
        Y = y;
        PSs = pSs;
    ...
    public GalacticCoordinates(ushort x, ushort z, byte y, byte g, ushort ss, byte p)
        : this(x, z, y, (ushort)((ss & 0x0FFF) | ((p & 0xF) << 12)), g)
    {
        if (ss > 4095) throw new ArgumentOutOfRangeException(nameof(ss), "Solar System Index must be in range 0 to 4095");
        if (p > 15) throw new ArgumentOutOfRangeException(nameof(p), p, "Planet Index must be in range 0 to 15");
    }
}
